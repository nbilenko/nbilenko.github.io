...
export default function PublicationsTalks() {
  ...
  return (
    <section>
      <div className="sticky top-0 md:top-20 left-4 max-w-6xl text-sm text-center border-b border-gray-200 dark:border-gray-700 bg-[#fbffff] dark:bg-[#1f4f5a]">
        <ul className="flex flex-wrap -mb-px">
          <li className="ml-2 md:ml-16 mr-2">
            <TabLink
              name="Publications"
              href="#publications"
              active={path.endsWith("#publications")}
            />
          </li>
          <li className="mr-2">
            <TabLink
  ...
  return (
    <li className="flex justify-between gap-x-6 py-5">
      <div className="gap-x-4">
        {pubColor && (
          <span
            className={`inline-flex items-center gap-1.5 py-1 px-2 rounded-md text-xs font-small bg-${pubColor}-100 text-${pubColor}-800`}
          >
            {type}
          </span>
        )}
        ...
        <div className="min-w-0 flex-auto">
          ...
          {topics && (
            <p className="mt-1 text-xs leading-5 text-gray-500">
              {topics.sort().map((topic, idx) => (
                <Badge key={idx} text={topic} />
              ))}
            </p>
          )}
        </div>
    ...
    <li className="flex justify-between gap-x-6 py-5">
      <div className="gap-x-4">
        ...
        <div className="min-w-0 flex-auto">
          ...
          {whereurl ? (
            <a href={whereurl} target="_blank" rel="noreferrer noopener">
              <p className="mt-1 text-xs leading-5 text-gray-500 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600 hover:text-sky-700 dark:hover:text-sky-400">
                {where}
              </p>
            </a>
          ) : (
